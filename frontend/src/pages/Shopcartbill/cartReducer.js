...
const cartReducer = (state=initalState, action) => {
    ...
    case REMOVE_FROM_CART:
      let itemToRemove= state.cartItem && state.cartItem.find(item=> action.id === item.id)
      let new_items = state.cartItem && state.cartItem.filter(item=> action.id !== item.id)
    ...
    case INCREAMENT :
      ...
      let decresedItem = state.cartItem && state.cartItem.find(item=> item.id === action.id) 
